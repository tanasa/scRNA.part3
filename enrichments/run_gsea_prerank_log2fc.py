...
Outputs go to:           GSEA_prerank_log2fc/
"""
...
OUT = BASE / "GSEA_prerank_log2fc"
DESIGN_TOKEN = "__group_age_z_sex_apoE__"

RANK_COL = "log2FoldChange"     # user-chosen ranking metric
...
def main() -> None:
    ...
    for csv_path in files:
        ...
        try:
            ...
            pd.DataFrame(log_rows).to_csv(OUT / "SUMMARY_gsea_prerank_log2fc_log.csv", index=False)
        except Exception:
    ...
    pd.DataFrame(log_rows).to_csv(OUT / "SUMMARY_gsea_prerank_log2fc_log.csv", index=False)
    if errors:
        (OUT / "SUMMARY_gsea_prerank_log2fc_errors.txt").write_text("\n\n".join(errors))
